...
\begin{problem} Asymptotics \end{problem}
    ...
    \begin{enumerate}   
        ...
        $n (\log n)^2 \in O\left(n^{1.5}(\log n)\right)$.
            ...
            \solution{ True.  We must show $\exists c>0,n_0>0$ such that $\forall n > n_0, n (\log n)^2 \le c n^{1.5}\log n.$ Let $c = 1$ and $n_0 = 1$, then
            \begin{eqnarray*}
                n (\log n)^2 &\le& c n^{1.5}\log n\\
                n (\log n)^2 &\le& n^{1.5}\log n\\
                n (\log n)(\log n) &\le& n^{1.5}\log n\\
                (\log n)(\log n) &\le& n^{1/2}\log n\\
                (\log n) &\le& n^{1/2}.
            \end{eqnarray*}
            For all $n > 1$, $\log n \le \sqrt{n}$ and therefore $n (\log n)^2 \in O(n^{1.5}\log n).$}
